...
const triggerBrowserReflow = (node: HTMLElement) => get(node, "offsetHeight");
...
function defaultGetDimensionValue(
  dimension: DIMENSION,
  elem: HTMLElement,
): number {
...
const Collapse = React.forwardRef(
  (props: CollapseProps, ref: React.Ref<unknown>) => {
    const {
      className,
      timeout = 300,
    ...
    const dimension =
      typeof dimensionProp === "function" ? dimensionProp() : dimensionProp;
    ...
    const handleExit = useCallback(
      (elem: HTMLElement) => {
        const value = getDimensionValue
          ? getDimensionValue(dimension, elem)
          : 0;
    ...
    return (
      <Transition
        ...
        className={merge(
          className,
          prefix({ "collapse-horizontal": dimension === "width" }),
        )}
