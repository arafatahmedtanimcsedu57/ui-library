 ...
 * @private
 */
const NavbarItem: RsRefForwardingComponent<"a", NavbarItemProps> =
  React.forwardRef((props: NavbarItemProps, ref: React.Ref<unknown>) => {
    const {
    ...
    const { activeKey, onSelect: onSelectFromNav } = useContext(
      NavContext,
    ) as NavContextProps;

    const active =
      activeProp ?? (!isNil(eventKey) && shallowEqual(eventKey, activeKey));
