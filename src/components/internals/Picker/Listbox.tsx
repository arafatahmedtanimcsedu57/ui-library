...
const Listbox: ListboxComponent = React.forwardRef<
  HTMLDivElement,
  ListboxProps<any>
>((props, ref) => {
  const {
  ...
  const classes = merge(
    className,
    withClassPrefix("items", { grouped: group }),
  );
  ...
  const handleGroupTitleClick = useEventCallback(
  ...
  const renderItem = ({
    ...
    if (group && item[RSUITE_PICKER_GROUP_KEY]) {
      ...
      isUndefined(value) &&
      !isUndefined(item[RSUITE_PICKER_GROUP_KEY])
    ) {
      throw Error(
        `valueKey "${valueKey}" is not defined in "data" : ${index} `,
      );
    }
    ...
    const focus =
      !isUndefined(focusItemValue) && shallowEqual(focusItemValue, value);
  ...
  useMount(() => {
    const itemIndex = findIndex(
      filteredItems,
      (item) => item[valueKey] === activeItemValues?.[0],
    );
