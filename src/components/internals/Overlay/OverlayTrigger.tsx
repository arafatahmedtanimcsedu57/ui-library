...
function mergeEvents(events = {}, props = {}) {
  ...
  Object.keys(events).forEach((eventName) => {
    if (events[eventName]) {
      nextEvents[eventName] = createChainedFunction(
        events[eventName],
        props?.[eventName],
      );
    }
...
function onMouseEventHandler(
  ...
  const related =
    event.relatedTarget || get(event, ["nativeEvent", "toElement"]);

  if (
    (!related || related !== target) &&
    !contains(target as HTMLElement, related)
  ) {
    handler(event, delay);
  }
}
...
const OverlayTrigger = React.forwardRef(
  (props: OverlayTriggerProps, ref: React.Ref<OverlayTriggerHandle>) => {
    ...
    useImperativeHandle(ref, () => ({
      ...
      open: handleOpen,
      close: (delay?: number) =>
        handleClose(delay, OverlayCloseCause.ImperativeHandle),
      updatePosition: () => {
    ...
    const triggerEvents = useMemo(() => {
      ...
      if (followCursor) {
        events.onMouseMove = createChainedFunction(
          handledMoveOverlay,
          onMouseMove,
        );
      }
      ...
      if (isOneOf("hover", trigger)) {
        const onMouseOverListener = (e) =>
          onMouseEventHandler(handleDelayedOpen, e);
        const onMouseOutListener = (e) =>
          onMouseEventHandler(handleDelayedClose, e);

        events.onMouseOver = createChainedFunction(
          onMouseOverListener,
          events.onMouseOver,
        );
        events.onMouseOut = createChainedFunction(
          onMouseOutListener,
          events.onMouseOut,
        );
      }
      ...
      if (isOneOf("focus", trigger)) {
        events.onFocus = createChainedFunction(
          handleDelayedOpen,
          events.onFocus,
        );
        events.onBlur = createChainedFunction(
          handleDelayedClose,
          events.onBlur,
        );
      }
    ...
    const renderOverlay = () => {
      const overlayProps: Omit<OverlayProps, "children"> = {
        ...
        onExited: createChainedFunction(
          followCursor ? handleExited : undefined,
          onExited,
        ),
        placement,
