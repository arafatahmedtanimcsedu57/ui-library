...
>((props, ref) => {
   ...
   *
   */
  const handleFocusDialog = useEventCallback(
    (onBeforeFocusCallback?: () => void) => {
      ...
      if (
        ...
        !contains(dialog, currentActiveElement)
      ) {
  ...
  const handleOpen = useEventCallback(() => {
    ...
    if (!documentKeyDownListener.current) {
      documentKeyDownListener.current = on(
        document,
        "keydown",
    ...
    if (!documentFocusListener.current) {
      documentFocusListener.current = on(
        document,
        "focus",
        handleEnforceFocus,
        true,
  ...
  const renderBackdrop = () => {
    ...
    return (
      <div aria-hidden style={backdropStyle} className={backdropClassName} />
    );
  };
