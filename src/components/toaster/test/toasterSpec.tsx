...
describe("toaster", () => {
  ...
  it("Should render 2 containers", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        element.querySelector(
          ".bdris-toast-container.bdris-toast-container-top-end",
        ),
      ).to.exist;
    });
    ...
    await waitFor(() => {
      ...
      expect(
        element.querySelector(
          ".bdris-toast-container.bdris-toast-container-bottom-end",
        ),
      ).to.exist;
    });
  });
  ...
  it("Should remove a message", async () => {
    ...
    await waitFor(() => {
      expect(
        (screen.queryByTestId("message") as HTMLElement).className,
      ).to.contain("rs-toast-fade-exiting");
    });
  ...
  it("Should clear all message", async () => {
    ...
    await waitFor(() => {
      expect(
        (screen.queryByTestId("message1") as HTMLElement).className,
      ).to.contain("rs-toast-fade-exiting");
      expect(
        (screen.queryByTestId("message2") as HTMLElement).className,
      ).to.contain("rs-toast-fade-exiting");
    });
