 ...
 */
const DropdownItem: RsRefForwardingComponent<"li", DropdownMenuItemProps> =
  React.forwardRef((props: DropdownMenuItemProps, ref: React.Ref<unknown>) => {
    const {
    ...
    const selected =
      activeProp ||
      (!isNil(eventKey) && shallowEqual(dropdown?.activeKey, eventKey));
    ...
    if (divider) {
      return (
        <DropdownSeparator
          as="li"
          {...(pick(props, ["data-testid"]) as DropdownSeparatorProps)}
        />
      );
    }
    ...
    return (
      ...
      >
        {({ selected, active, ...menuitem }, menuitemRef) => {
          ...
          const dataAttributes: { [key: string]: unknown } = {
