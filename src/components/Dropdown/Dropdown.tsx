...
export interface DropdownComponent
  extends RsRefForwardingComponent<"div", DropdownProps> {
...
const Dropdown: DropdownComponent = React.forwardRef<HTMLElement>(
  (props: DropdownProps, ref) => {
    ...
    const renderMenuButton = (
      menuButtonProps: unknown,
      menuButtonRef: unknown,
    ) => (
      <DropdownToggle
        ref={menuButtonRef}
        as={toggleAs}
      ...
      <DropdownContext.Provider value={dropdownContextValue}>
        <Menu
          ...
          renderMenuPopup={({ open, ...popupProps }, popupRef) => {
            const menuClassName = mergeMenuClassName(
              className,
              withMenuClassPrefix({}),
            );
        ...
        >
          {(
            { open, ...menuContainer },
            menuContainerRef: React.Ref<HTMLElement>,
          ) => {
            const classes = merge(
              className,
              withClassPrefix({
                [`placement-${kebabCase(placementPolyfill(placement))}`]:
                  !!placement,
                disabled,
                open,
