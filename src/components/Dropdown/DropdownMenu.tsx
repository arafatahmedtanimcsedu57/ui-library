...
>((props, ref) => {
  ...
  if (nav) {
    warnOnce(
      "Usage of <Dropdown.Menu> within <Nav> is deprecated. Replace with <Nav.Menu>",
    );

    return <Nav.Menu ref={ref} {...(props as unknown)} />;
  }
      ...
      renderMenuButton={({ open, ...menuButtonProps }, buttonRef) => (
        <MenuItem disabled={disabled}>
          {({ selected, active, ...menuitem }, menuitemRef) => {
            ...
            return (
              <div
                ref={mergeRefs(buttonRef, menuitemRef as unknown)}
                className={classes}
              ...
              >
                {icon &&
                  React.cloneElement(icon, { className: prefix("menu-icon") })}
                {title}
